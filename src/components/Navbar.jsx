  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      for (let i = 0; i < allSections.length; i++) {
        ...
        if (
          allSections[i].id !== "" &&
          currentScrollPos >= sectionTopPos &&
          currentScrollPos < sectionBottomPos
        ) {
      ...
      >
        <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
          ...
          >
            ...
            />
            <p className="font-bold cursor-pointer text-[1.4rem]">Vinay</p>
          </ScrollLink>
          ...
          <ul className="ul-list list-none sm:flex flex gap-10 items-center">
            {navLinks.map((link) => (
              <li
                key={link.id}
                className={`${
                  currentPath === link.id ? "text-[#915eff]" : "text-secondary"
                } cursor-pointer ease-in-out duration-100 hover:text-[#915eff]`}
              >
                <ScrollLink
                  to={link.id}
                  spy={true}
                  ...
                  className={`${
                    currentPath === link.id ? "text-[1.2rem]" : ""
          ...
          <div className="sm:hidden flex justify-center items-center flex-col relative">
            ...
            >
              <img
                ...
                src={menuImgSrc}
                alt="menu"
                className="w-[1.1rem] h-[1.1rem] object-contain cursor-pointer"
                onClick={() => {
                  toggleMenuImg();
                  setmenuOn(!menuOn);
                }}
            ...
            >
              <div
                ref={menuContainer}
                className="flex bg-gradient-to-br from-black from-50% to-violet-500 rounded-lg p-6 absolute z-10 top-5 right-5"
              >
